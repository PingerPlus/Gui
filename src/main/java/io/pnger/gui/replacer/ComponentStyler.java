...
public class ComponentStyler {
    ...
    public static Component linesToComponent(@Nonnull String... lines) {
        return ComponentStyler.linesToComponent(Arrays.asList(lines));
    }
    ...
    public static Component linesToComponent(List<String> lines) {
        return lines.stream()
            .map(ComponentStyler::componentFromContent)
            .collect(Collectors.collectingAndThen(Collectors.toList(), ComponentStyler::mergeComponents));
    }
    ...
    public static Component mergeComponents(@Nonnull Component... components) {
        return ComponentStyler.mergeComponents(Arrays.asList(components));
    }
    ...
    public static List<String> contentFromComponent(@Nonnull List<Component> components) {
        return components.stream().map(ComponentStyler::contentFromComponent).collect(Collectors.toList());
    }
